...
class Life(Board):
    ...
    def who_is_alive(self, x, y):
        count = 0
        for i in range(-1, 2):
            for j in range(-1, 2):
                if (i != 0 or j != 0) and 0 <= x + i < self.width and 0 <= y + j < self.height:
                    count += self.board[y + j][x + i]
        if count == 3 and self.board[y][x] == 0:
            return 1
        if (count == 2 or count == 3) and self.board[y][x] == 1:
            return 1
        return 0
    ...
    def next_move(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                tmp_board[y][x] = self.who_is_alive(x, y)
